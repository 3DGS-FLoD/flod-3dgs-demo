...
export class FLoDPlyParser {
    static maxLevel = 5;
    static lod1ScalingLowerBound = 0.2;
    static lodScalingRatio = 4.0;
    ...
    static decodeHeaderText(headerText) {
        ...
        const header = FLoDPlyParser.decodeHeaderLines(headerLines);
    ...
    static decodeHeaderFromBuffer(plyBuffer) {
        ...
        return FLoDPlyParser.decodeHeaderText(headerText);
    }
    ...
    static parseToUncompressedSplatBufferSection(header, fromSplat, toSplat, splatData, splatDataOffset,
                                                 toBuffer, toOffset, outSphericalHarmonicsDegree = 0, lodLevel) {
        ...
        for (let i = fromSplat; i <= toSplat; i++) {
            const parsedSplat = FLoDPlyParser.parseToUncompressedSplat(splatData, i, header,
                                                                          splatDataOffset, outSphericalHarmonicsDegree, lodLevel);
    ...
    static parseToUncompressedSplatArraySection(header, fromSplat, toSplat, splatData, splatDataOffset,
                                         splatArray, outSphericalHarmonicsDegree = 0, lodLevel) {
        outSphericalHarmonicsDegree = Math.min(outSphericalHarmonicsDegree, header.sphericalHarmonicsDegree);
        for (let i = fromSplat; i <= toSplat; i++) {
            const parsedSplat = FLoDPlyParser.parseToUncompressedSplat(splatData, i, header,
                                                                          splatDataOffset, outSphericalHarmonicsDegree, lodLevel);
    ...
    static decodeSectionSplatData(sectionSplatData, splatCount, sectionHeader, outSphericalHarmonicsDegree, toSplatArray = true, lodLevel) {
        ...
        if (toSplatArray) {
            ...
            for (let row = 0; row < splatCount; row++) {
                const newSplat = FLoDPlyParser.parseToUncompressedSplat(sectionSplatData, row, sectionHeader,
                                                                           0, outSphericalHarmonicsDegree, lodLevel);
            ...
            FLoDPlyParser.parseToUncompressedSplatBufferSection(
                sectionHeader, 0, splatCount - 1, sectionSplatData, 0,
                splatBuffer.bufferData, splatBufferDataOffsetBytes, outSphericalHarmonicsDegree, lodLevel
            );
            return splatBuffer;
        }
    }
    ...
    static parseToUncompressedSplat = function() {
        ...
        return function(splatData, row, header, splatDataOffset = 0, outSphericalHarmonicsDegree = 0, lodLevel) {
            outSphericalHarmonicsDegree = Math.min(outSphericalHarmonicsDegree, header.sphericalHarmonicsDegree);
            FLoDPlyParser.readSplat(splatData, header, row, splatDataOffset, rawSplat);
            ...
            if (lodLevel !== FLoDPlyParser.maxLevel) {
                const scalingLowerBound = FLoDPlyParser.lod1ScalingLowerBound / Math.pow(FLoDPlyParser.lodScalingRatio, lodLevel - 1);
                newSplat[OFFSET_SCALE0] += scalingLowerBound;
                newSplat[OFFSET_SCALE1] += scalingLowerBound;
                newSplat[OFFSET_SCALE2] += scalingLowerBound;
            }
    ...
    static parseToUncompressedSplatArray(plyBuffer, outSphericalHarmonicsDegree = 0, lodLevel) {
        const { header, splatCount, splatData } = separatePlyHeaderAndData(plyBuffer);
        return FLoDPlyParser.decodeSectionSplatData(splatData, splatCount, header, outSphericalHarmonicsDegree, true, lodLevel);
    }

    static parseToUncompressedSplatBuffer(plyBuffer, outSphericalHarmonicsDegree = 0, lodLevel) {
        const { header, splatCount, splatData } = separatePlyHeaderAndData(plyBuffer);
        return FLoDPlyParser.decodeSectionSplatData(splatData, splatCount, header, outSphericalHarmonicsDegree, false, lodLevel);
    }
}

function separatePlyHeaderAndData(plyBuffer) {
    const header = FLoDPlyParser.decodeHeaderFromBuffer(plyBuffer);
    const splatCount = header.splatCount;
    const splatData = FLoDPlyParser.findSplatData(plyBuffer, header);
    return {
        header,
        splatCount,
        splatData
